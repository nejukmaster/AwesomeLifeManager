...
public static class Utility
{
    ...
    public static float pathFunction(float p_pos, float p_delta, Vector2 p_range, float alpha, float b)
    {
        float r = alpha * (p_pos + p_delta) + b;
        while (!(r >= p_range.x && r <= p_range.y))
        {
            if (p_range.x > r)
            {
                r = Mathf.Abs(r - p_range.x) + p_range.x;
            }
            else if (p_range.y < r)
            {
                r =  -1 * Mathf.Abs(r - p_range.y) + p_range.y;
            }
        }
        return r;
    }
